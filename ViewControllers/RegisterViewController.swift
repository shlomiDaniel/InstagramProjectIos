...
class RegisterViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
   ...
   var fireBaseDataBase = ModelFireBase()
    ...
    @IBAction func register(_ sender: Any) {
        ...
        let uid = user.uid
        let storage_ref = Storage.storage().reference(forURL: "gs://instagramfirebase-6b380.appspot.com").child("profile_image").child(uid)
             newUserRef.setValue(["userName": self.username_txt.text! , "email" : self.email_txt.text!])
            if let profile_imag =  self.image_selected!.jpegData(compressionQuality: 0.1){
            storage_ref.putData(profile_imag, metadata: nil) { (metadata, error) in
                if error != nil{
                    return
                }
                
                let profile_image_url = metadata?.storageReference?.downloadURL(completion: { (url, error) in
                    var image_url = url?.absoluteString
                    let newuserref = Database.database().reference().child("users").child(uid)
                    // : https://instagramfirebase-6b380.firebaseio.com/
